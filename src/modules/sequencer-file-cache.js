...
const SequencerFileCache = {
  ...
  unloadSpritesheet(inSrc) {
    ...
    // for (let i = packsSize - 1; i >= 0; i--) {
    //   const relatedSheet = relatedSheets[i]
    //   const relatedPack = relatedPacks[i]
    //   if (!relatedPack || !relatedSheet) {
    //     continue
    //   }
    //   const imageUrl = get_sheet_image_url(inSrc, relatedSheet)
    //     cacheKeys.push(imageUrl)
    //   const sheetSourceUrl = get_sheet_source_url(inSrc, relatedPack)
    //     cacheKeys.push(sheetSourceUrl)
    // }
